...
void ADXL::accel_setup(){
  powerOn();
  setFullResBit(1);        // 256 LSB/g
  setAxisOffset(ACC_X_OFFSET, ACC_Y_OFFSET, ACC_Z_OFFSET);
}
...
void ADXL::setAxisGains(double *_gains){
	int i;
	for(i = 0; i < 3; i++){
		gains[i] = _gains[i];
	}
}
void ADXL::getAxisGains(double *_gains){
	int i;
	for(i = 0; i < 3; i++){
		_gains[i] = gains[i];
	}
...
void ADXL::setDoubleTapLatency(int doubleTapLatency) {
...
void ADXL::setDoubleTapWindow(int doubleTapWindow) {
...
bool ADXL::triggered(byte interrupts, int mask){
